...
// Holding input and send command
byte Collect[9];
uint8_t command[8];
...
//    uint8_t M1 = Serial.read(); // read the uint8_t value from the serial port
    ...
    if (CAN_MSGAVAIL == CAN.checkReceive()) {         // check if data coming
        ...
        // Add condition to check for current v or p
        // Create some flag-> true so it go into if one time
        // Initiate brake for some second
        // use the milii loop structure
