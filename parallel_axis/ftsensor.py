...
class ftsensor:
    ...
    def __init__(
        self,
        calibration_file_path="FT44764\FT44764.cal",
        ...
        self.raw_readings = np.zeros(
            (self.channel_number, self.sample_number), dtype=np.float64
        )
    ...
    def set_tool(self, tool_offset):
        self.sensor.setToolTransform(
            tool_offset, "mm".encode("utf-8"), "deg".encode("utf-8")
        )
    ...
    def read_raw_ft(self):
        raw_voltage = self.read_raw_voltage()
        return [
            self.sensor.convertToFt(
                [raw_voltage[i][j] for i in range(self.channel_number)]
            )
